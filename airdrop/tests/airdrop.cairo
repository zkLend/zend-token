...
fn test_claim_single_recipient() {
    ...
    let airdrop = deploy_airdrop(token.contract_address, leaf);
    ...
    pop_log::<Transfer>(token.contract_address).unwrap();
    pop_log::<Transfer>(token.contract_address).unwrap();
    let log = pop_log::<Transfer>(token.contract_address).unwrap();
...
fn test_claim_two_claims() {
    ...
    let airdrop = deploy_airdrop(token.contract_address, root);
...
fn test_claim_two_claims_via_claim_128() {
    ...
    let transfer_claim_a_log = pop_log::<Transfer>(token.contract_address).unwrap();
    ...
    let transfer_claim_b_log = pop_log::<Transfer>(token.contract_address).unwrap();
...
fn test_claim_is_valid(root: felt252, claim: Claim, proof: Array<felt252>) {
    ...
    let token = deploy_token('AIRDROP', 'AD', claim.amount);
    let airdrop = deploy_airdrop(token.contract_address, root);
    ...
    // pops the transfer from deployer to airdrop
    pop_log::<Transfer>(token.contract_address).unwrap();
    let transfer_log = pop_log::<Transfer>(token.contract_address).unwrap();
...
fn test_double_claim_after_other_claim() {
    ...
    let token = deploy_token('AIRDROP', 'AD', claim_0.amount.into() + claim_1.amount.into());
...
fn test_claim_before_funded() {
    ...
    let token = deploy_token('AIRDROP', 'AD', 0);
...
fn test_claim_128_empty() {
    ...
    let airdrop = deploy_airdrop(token.contract_address, 0);
...
fn test_claim_128_large_tree() {
    ...
    let token = deploy_token('AIRDROP', 'AD', 960);
    let airdrop = deploy_airdrop(token.contract_address, root);
